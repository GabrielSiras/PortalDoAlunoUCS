<template>
  <div class="page-content">
    ...
    <div v-else class="semanal-card card">
      ...
      <div v-for="ev in eventos" :key="ev.id" class="ev-semanal">
        <div class="ev-date-col">
          <div class="ev-dia-sem">{{ ev.diaSemana }}</div>
          <div class="ev-dia-num">{{ ev.diaNum }}<br/><small>{{ ev.mes }}</small></div>
        </div>
    ...
    <div class="eventos-list">
      <div v-for="ev in eventos" :key="ev.id" class="evento-item card">
        <div class="ev-date-col">
          <div class="ev-dia-sem">{{ ev.diaSemana }}</div>
          <div class="ev-dia-num">{{ ev.diaNum }}<br/><small>{{ ev.mes }}</small></div>
        </div>
...
const mes = ref(4) // maio = 4 (0-indexed)
const ano = ref(2026)

const meses = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro']
const diasSemana = ['Dom','Seg','Ter','Quar','Quin','Sex','Sáb']

const nomeMes = computed(() => meses[mes.value])
...
const eventos = [
  { id: 1, diaSemana: 'Sexta', diaNum: '01', mes: 'Abril', dia: 1, nome: 'FERIADO – Dia dos trabalhadores', local: '', color: '#ef4444' },
  { id: 2, diaSemana: 'Quarta', diaNum: '06', mes: 'Abril', dia: 6, nome: 'IHC – Apresentação do trabalho – 19:40', local: '(Bloco 71, Sala 307)', color: '#f97316' },
  { id: 3, diaSemana: 'Sexta', diaNum: '08', mes: 'Abril', dia: 8, nome: 'Banco de Dados I – Trabalho Final', local: '(Bloco 71, Sala 203)', color: '#22c55e' },
]

function eventosNoDia(dia) {
  return eventos.filter(e => e.dia === dia)
}
</script>
