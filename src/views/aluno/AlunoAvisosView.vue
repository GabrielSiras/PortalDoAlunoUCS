...
const disciplinasUnicas = computed(() => {
  const ds = new Set(avisos.value.filter(a => a.disciplina).map(a => a.disciplina))
  return [...ds]
})

const avisosFiltrados = computed(() => {
  if (!filtroSelecionado.value) return avisos.value
  return avisos.value.filter(a => a.disciplina === filtroSelecionado.value)
})
